...
class MVTecDataset(object):
    def __init__(self, root, ext, train, mode=None, neg_dir=None, pos_dir=None, preprocessor=None):
        if train:
            if not neg_dir:
            ...
            if mode == 'pos' and (not pos_dir and not neg_dir):
                raise ValueError("The argument 'neg_dir' must be set in order to consider data contained in other directories as positive samples")
...
class DataLoader(object):
    ...
    def __next__(self):
        ...
        if self.counter + self.batch_size > len(self.dataset) and self.drop_last == True:
            self.counter = 0
            raise StopIteration()
