...
class SparseCodingWithMultiDict(object):
    ...
    def visualize(self, ch, org_H, org_W, patch_size, stride):
        coder = SparseCoder(
            dictionary=self.dictionaries[ch],
            transform_algorithm=self.transform_algorithm,
            transform_n_nonzero_coefs=self.num_of_nonzero,
        )
        ...
        self.visualize_features(
            ...
            stride=stride,
            desc="visualizing for negative sample",
        )
        self.visualize_features(
            coder=coder,
            mode="pos",
            ...
            patch_size=patch_size,
            stride=stride,
            desc="visualizing for positive sample",
        )

    def visualize_features(
        self, coder, mode, ch, org_H, org_W, patch_size, stride, desc=None
    ):
        if mode == "neg":
            loader = self.test_neg_loader
        elif mode == "pos":
